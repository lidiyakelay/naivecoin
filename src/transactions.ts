...
const validateBlockTransactions = (aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[], blockIndex: number): boolean => {
    ...
    // Check for duplicate txIns using a Set
    const txIns: TxIn[] = aTransactions.flatMap(tx => tx.txIns); // Use flatMap to flatten the array
    ...
    return normalTransactions.map((tx) => validateTransaction(tx, aUnspentTxOuts))
        .every(valid => valid); // Use every instead of reduce for readability
};

const hasDuplicates = (txIns: TxIn[]): boolean => {
    const counts: { [key: string]: number } = {}; // Object to count occurrences

    // Count each txIn based on its txOutId and txOutIndex combination
    for (const txIn of txIns) {
        const key = txIn.txOutId + txIn.txOutIndex; // Create a unique key
        counts[key] = (counts[key] || 0) + 1; // Increment the count
    }
...
const validateCoinbaseTx = (transaction: Transaction, blockIndex: number): boolean => {
    ...
    if (transaction.txIns[0].txOutIndex !== 0) {
